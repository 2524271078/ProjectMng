...
const contracts = ref([]); const overview = ref(null); const drawerVisible = ref(false); const dialogVisible = ref(false)
const form = reactive({ contract_no: '', contract_name: '', final_customer: 1, amount: 0 })
async function loadContracts() { const { data } = await listResource('contracts'); contracts.value = data.results || data }
async function createContract() { await createResource('contracts', form); dialogVisible.value = false; await loadContracts() }
async function openDetail(row) { const { data } = await fetchContractOverview(row.id); overview.value = data; drawerVisible.value = true }
